#ifndef LINT
static char *rcsid="$Id: eph_open.c,v 2.17 1999/03/06 13:37:08 crosser Exp crosser $";
#endif
...
/*
	pjt: Added return codes so that the
	application can try other ports and speeds automatically.
	Changes were only made to the WINDOWS code path:

	-1 = Programming error (change code)
	-2 = Cannot open device (try next device)
	-3 = Cannot retrieve device configuration (try next device)
	-4 = Cannot configure device (try next device)
	-5 = Cannot set speed (try slower speed)
*/
...
#error platform not defined
	...
	if (iob->debug) EPHTRACE("set timeout to %lu\n",DATATIMEOUT+iob->timeout);
	...
	usleep(1);
	if (iob->debug)
		EPHTRACE("delay factor: %g loops/usec\n",(float)delay_factor);
...
#error platform not defined
	...
	wsprintf(commtext, "%d,n,8,1", DEFSPEED);
